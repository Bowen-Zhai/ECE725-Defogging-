...
def regularization(row, col, transmission_estimation, img_hazy_corrected, n_points, air, cluster_Points, cluster):
    ...
    bin_count_map = np.zeros((row, col), int)
    radius_std = np.zeros((row, col), float)
    K_std = np.zeros(n_points, float)
    ...
    transmission = wlsFilter.wls_filter(trans, data_term_weight, img_hazy_corrected.astype(np.float32), lambd)
    return transmission